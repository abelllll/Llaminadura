...
{
	projeccio = ORTO; // Asigna la proyección ortográfica
	InvalidateRect(NULL, false); // Fuerza redibujado de la escena
}
...
// Función que calcula la distancia de la cámara (OPV.R)
void CEntornVGIView::CalcularDistanciaCamara(const glm::vec3& minExtremos, const glm::vec3& maxExtremos, GLuint sh_programID)
{
	// Obtener la diagonal del objeto (D)
	float D = glm::distance(minExtremos, maxExtremos); // Distancia entre los extremos del objeto

	// Calcular el radio de la esfera envolvente
	float r = D / 2.0f;

	// Obtener el ángulo de visión (field of view) en radianes
	float alpha = glm::radians(45.0f); // Puedes obtener este valor de la matriz de proyección si es dinámico

	// Calcular la distancia de la cámara usando la fórmula
	float R = r / sin(alpha / 2);

	// Ajustar la distancia de la cámara en la variable OPV.R
	OPV.R = R;

	// Llamar a la función de vista esférica con el nuevo valor de OPV.R
	Vista_Esferica(sh_programID, OPV_G, true, true, minExtremos, maxExtremos);
}
