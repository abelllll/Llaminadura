...
{
	// 1) Viewport y (si está activo) scissor del CUADRANTE
	glViewport(minx, miny, w, h);
	glScissor(minx, miny, w, h);   // requiere GL_SCISSOR_TEST habilitado antes

	// 2) Aspect ratio del cuadrante
	const float aspect = (h > 0) ? static_cast<float>(w) / static_cast<float>(h) : 1.0f;

	// 3) Tamaño base del volumen ortográfico (tu "zoom")
	//    Sube o baja este valor para ver más o menos escena.
	const float size = 6.0f;

	// 4) Mantener proporciones: expandimos el eje necesario según 'aspect'
	float halfX, halfY;
	if (aspect >= 1.0f) {
		// Viewport más ancho que alto → ampliamos X
		halfX = size * aspect;
		halfY = size;
	}
	else {
		// Viewport más alto que ancho → ampliamos Y
		halfX = size;
		halfY = size / aspect;
	}
	...
	// Profundidad amplia para ortográfica (ajusta si lo necesitas)
	const float nearVal = -10000.0f;
	const float farVal = 10000.0f;

	glm::mat4 ProjMatrix = glm::ortho(left, right, bottom, top, nearVal, farVal);

	// 5) Subir la proyección al shader (usa el nombre de tu uniform)
	GLint loc = glGetUniformLocation(sh_programID, "projection");
	if (loc == -1) {
		// Por si en tus shaders se llama 'projectionMatrix'
		loc = glGetUniformLocation(sh_programID, "projectionMatrix");
	}
	if (loc != -1) {
...
// Iluminacio movent-se amb la camara (abans glm::lookAt() )
	...
	switch (prj)
	{
	case PLANTA:  // Vista des de dalt (eix Y)
		MatriuVista = glm::lookAt(
			glm::vec3(0.0f, 10.0f, 0.0f),    // eye
			glm::vec3(0.0f, 0.0f, 0.0f),     // center
			glm::vec3(0.0f, 0.0f, -1.0f)     // up
		);
		break;

	case ALÇAT:  // Vista frontal (YZ)
		MatriuVista = glm::lookAt(
...
void FonsN()
{
	...
	CRect rect;
	CWnd* pWnd = CWnd::GetFocus();   // obtener ventana activa
	if (pWnd) {
		pWnd->GetClientRect(&rect);     // tamaño actual de la ventana
		int ample = rect.Width();       // ancho
		int alt = rect.Height();        // alto

	}	// Aquí irán luego los 4 viewports
}
